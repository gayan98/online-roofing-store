<?php
	...
	if (isset($_POST["up_shipping"]))
    {
        $id = $_POST["id"];

        $connection = mysqli_connect("localhost", "root", "", "roofing");

        $sql = "UPDATE `main_order` SET `order_status`=\"Delivered\" WHERE order_id='$id'";
        $result = mysqli_query($connection, $sql);
        exit();
    }
?>
...
<body class="animsition">
	...
	<div class="container">
	    <table class="table">
			...
			<td>
	            <button class = "btn btn-success" onclick="up_shipping_data(this.getAttribute('data-id'));" data-id="<?php echo $row->order_id; ?>">
	                Confirm
	            </button>
	        </td>
	            </tr>
	...
	function up_shipping_data(id) {
        console.log(id);
        $.ajax({
            url: "sup_orders.php",
            method: "POST",
            data: {up_shipping: 1, id: id},
            success: function () {
